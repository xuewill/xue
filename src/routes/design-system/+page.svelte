<script lang="ts">
  ...
  const colors: Token[] = [
    { name: 'Paper', value: 'var(--webase-color-canvas)', note: 'Primary canvas and the light theme ground.', sample: 'paper', kind: 'color' },
    { name: 'Surface', value: 'var(--webase-color-surface)', note: 'Raised paper for cards and controls.', sample: 'surface', kind: 'color' },
    { name: 'Ink', value: 'var(--webase-color-ink)', note: 'Long-form reading and high-contrast headings.', sample: 'ink', kind: 'color' },
    { name: 'Ink muted', value: 'var(--webase-color-ink-muted)', note: 'Secondary metadata and quiet instructions.', sample: 'muted', kind: 'color' },
    { name: 'Blue mark', value: 'var(--webase-color-brand)', note: 'Links, active states, and the registration mark.', sample: 'brand', kind: 'color' },
    { name: 'Blue wash', value: 'var(--webase-color-brand-tint)', note: 'A soft field behind selected controls.', sample: 'tint', kind: 'color' }
  ];

  const statusTokens: Token[] = [
    { name: 'Error', value: 'var(--webase-color-error)', note: 'Validation failures and destructive outcomes.', sample: 'status-error', kind: 'color' },
    { name: 'Success', value: 'var(--webase-color-success)', note: 'Completed saves and healthy metadata.', sample: 'status-success', kind: 'color' },
    { name: 'Warning', value: 'var(--webase-color-warning)', note: 'Work in progress and experimental marks.', sample: 'status-warning', kind: 'color' },
    { name: 'Info', value: 'var(--webase-color-info)', note: 'Neutral notices that need no action.', sample: 'status-info', kind: 'color' }
  ];

  const typeTokens: Token[] = [
    { name: 'Reading serif', value: 'var(--webase-font-body)', note: 'Charter / Palatino / Georgia fallback for essays.', sample: 'The quiet archive', kind: 'type' },
    { name: 'Display italic', value: 'var(--webase-font-display)', note: 'Cormorant for a small editorial accent.', sample: 'A note in the margin', kind: 'type' },
    { name: 'Utility sans', value: 'var(--webase-font-sans)', note: 'Futura for navigation, controls, and labels.', sample: 'PUBLISHED / 2026', kind: 'type' },
    { name: 'Annotation mono', value: 'var(--webase-font-mono)', note: 'Coordinates, token values, and system notes.', sample: 'space-24 · 24px', kind: 'type' }
  ];
  ...
  const borderTokens: Token[] = [
    { name: 'Hairline', value: '1px solid var(--webase-color-border)', note: 'Quiet edges around paper surfaces.' },
    { name: 'Strong hairline', value: '1px solid var(--webase-color-border-strong)', note: 'Interactive edges and field boundaries.' },
    { name: 'Dot rule', value: 'radial-gradient(circle, ...)', note: 'The archive and chapter divider signature.' },
    { name: 'Focus ring', value: '2px solid var(--webase-color-brand)', note: 'Visible keyboard focus with a 3–4px offset.' }
  ];
...
<main class="ds-page">
    ...
    <div class="ds-sidebar-inner">
      ...
      <nav>
        {#each sections as section (section.id)}
          <WeBaseButton
            size="sm"
            variant={activeSection === section.id ? 'ink' : 'text'}
  ...
  <div class="ds-content">
    <section id="overview" class="ds-hero ds-section">
      <div class="ds-hero-grid">
        <div>
          ...
          <div class="ds-hero-ledger" aria-label="Design system coverage"><span><strong>27</strong> components</span><span><strong>{sections.length}</strong> sections</span><span><strong>3</strong> input modes</span></div>
        </div>
      ...
      <div class="ds-overview-notes">
        <p><strong>Scope</strong> This living catalogue is the source of truth for the public archive and the shared WeBaseUI package.</p>
    ...
    <section id="components" class="ds-section">
      ...
      <div class="ds-component-ledger"><span>08 specimen sheets</span><span>Default + focus + disabled + error</span><WeBaseBadge label="Expanded" dot /></div>
      ...
      <article class="ds-specimen-sheet">
        ...
        <div class="ds-specimen-canvas ds-navigation-stack">
          ...
          <div class="ds-control-row">
            <WeBaseLink href="#components" label="Inline link" />
            <WeBaseLink variant="back" href="#overview" label="Back to overview" />
            <WeBaseLink variant="action" href="/archive" label="Open archive" />
          </div>
          <div class="ds-control-row">
            <WeBaseTag label="Field note" />
            <WeBaseTag variant="brand" label="Design" count="12" />
            <WeBaseTag variant="outline" href="/blog/tags/design" label="Browse topic" />
          </div>
    ...
    <section id="motion" class="ds-section">
      ...
      <div class="ds-motion-proof">
        <div class="ds-motion-toolbar">
          ...
          <WeBaseButton label="Replay motion sequence" icon="sparkles" onclick={() => (motionReplay += 1)} />
        </div>
        {#key motionReplay}
          <div class="ds-motion-grid" data-motion-run={motionReplay}>
            <article>
              ...
              <div><small>01 / Response</small><strong>Press / 140ms</strong><code>var(--webase-duration-fast)</code><p>A control moves only far enough to confirm contact.</p></div>
            </article>
            <article>
              <div class="ds-motion-visual ds-motion-reveal" aria-hidden="true"><i></i><span>Field note</span></div>
              <div><small>02 / Reveal</small><strong>Uncover / 200ms</strong><code>var(--webase-duration-ui)</code><p>New content enters from the edge that owns it.</p></div>
            </article>
            <article>
              <div class="ds-motion-visual ds-motion-register" aria-hidden="true"><i></i><b></b><span></span></div>
              <div><small>03 / Align</small><strong>Register / 200ms</strong><code>var(--webase-ease-out)</code><p>Selection resolves with a short, precise snap.</p></div>
            </article>
            <article>
    ...
    <section id="materials" class="ds-section">
      ...
      <footer class="ds-endnote"><p>The public archive now shares these tokens and primitives while retaining its own editorial compositions.</p><WeBaseLink variant="action" href="/" label="Return to the archive" /></footer>
    </section>
  </div>
...
<style>
  ...
  .ds-stamp { display: inline-flex; width: max-content; padding: 6px 8px; border: 1px solid var(--webase-color-brand); color: var(--webase-color-brand); font-family: var(--webase-font-mono); font-size: 10px; letter-spacing: .12em; transform: rotate(-2deg); }
  .ds-sidebar nav { display: grid; gap: 2px; }
  .ds-sidebar-foot { margin: 0; color: var(--webase-color-ink-muted); font-family: var(--webase-font-display); font-size: 18px; font-style: italic; line-height: 1.15; }
  ...
  .ds-kicker { margin: 0 0 16px; color: var(--webase-color-brand); font-family: var(--webase-font-mono); font-size: 10px; letter-spacing: .14em; text-transform: uppercase; }
  h1, p { margin-top: 0; }
  h1 { max-width: 700px; margin-bottom: 24px; font-family: var(--webase-font-body); font-size: clamp(54px, 9vw, 112px); font-weight: 500; letter-spacing: -.075em; line-height: .83; }
  .ds-cursor { display: inline-block; margin-left: 6px; color: var(--webase-color-brand); font-family: var(--webase-font-mono); font-size: .4em; letter-spacing: 0; transform: translateY(-.15em); }
  .ds-lede { max-width: 520px; margin-bottom: 0; color: var(--webase-color-ink-muted); font-size: clamp(17px, 2.3vw, 22px); line-height: 1.45; }
  .ds-hero-ledger { display: flex; flex-wrap: wrap; gap: 0; margin-top: 34px; border-top: 1px solid var(--webase-color-border); border-bottom: 1px solid var(--webase-color-border); }
  .ds-hero-ledger span { display: inline-flex; min-height: 48px; align-items: baseline; gap: 7px; padding: 0 16px; border-right: 1px solid var(--webase-color-border); color: var(--webase-color-ink-muted); font-family: var(--webase-font-mono); font-size: 9px; letter-spacing: .08em; text-transform: uppercase; }
  .ds-hero-ledger span:first-child { padding-left: 0; }
  .ds-hero-ledger strong { color: var(--webase-color-brand); font-family: var(--webase-font-body); font-size: 25px; font-weight: 500; letter-spacing: -.04em; }
  .ds-registration { position: relative; display: grid; width: 210px; min-height: 240px; align-content: space-between; justify-items: center; justify-self: end; padding: 18px; border: 1px solid var(--webase-color-border-strong); color: var(--webase-color-brand); background: var(--webase-color-surface); box-shadow: 9px 9px 0 color-mix(in srgb, var(--webase-color-brand) 12%, transparent); transform: rotate(1.2deg); }
  .ds-registration::before, .ds-registration::after { position: absolute; width: 14px; height: 14px; border-color: var(--webase-color-brand); content: ''; opacity: .55; }
  ...
  .ds-registration span { justify-self: start; font-family: var(--webase-font-mono); font-size: 9px; letter-spacing: .14em; }
  .ds-registration img { width: 112px; height: 112px; opacity: .94; }
  .ds-registration small { justify-self: start; color: var(--webase-color-ink-muted); font-family: var(--webase-font-mono); font-size: 9px; letter-spacing: .08em; line-height: 1.5; text-transform: uppercase; }
  .ds-overview-notes { display: grid; grid-template-columns: repeat(2, 1fr); gap: 32px; padding-top: 30px; }
  .ds-overview-notes p, .ds-note { margin: 0; color: var(--webase-color-ink-muted); font-size: 14px; line-height: 1.55; }
  .ds-overview-notes strong { display: block; margin-bottom: 8px; color: var(--webase-color-ink); font-family: var(--webase-font-sans); font-size: 11px; letter-spacing: .1em; text-transform: uppercase; }
  ...
  .ds-color-sample { height: 90px; border: 1px solid var(--webase-color-border); }
  .ds-color-paper { background: var(--webase-color-canvas); }
  .ds-color-surface { background: var(--webase-color-surface); }
  .ds-color-ink { background: var(--webase-color-ink); }
  .ds-color-muted { background: var(--webase-color-ink-muted); }
  .ds-color-brand { background: var(--webase-color-brand); }
  .ds-color-tint { background: var(--webase-color-brand-tint); }
  .ds-color-status-error { background: var(--webase-color-error); }
  .ds-color-status-success { background: var(--webase-color-success); }
  .ds-color-status-warning { background: var(--webase-color-warning); }
  .ds-color-status-info { background: var(--webase-color-info); }
  .ds-status-row { display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); gap: 14px; margin-top: 22px; padding-top: 22px; border-top: 1px dashed var(--webase-color-border-strong); }
  .ds-status-token { display: grid; grid-template-columns: 26px minmax(0, 1fr); gap: 12px; min-width: 0; }
  .ds-status-chip { width: 26px; height: 26px; border: 1px solid var(--webase-color-border); border-radius: 50%; }
  .ds-status-token strong { display: block; margin-bottom: 4px; color: var(--webase-color-ink); font-family: var(--webase-font-sans); font-size: 11px; letter-spacing: .06em; text-transform: uppercase; }
  .ds-status-token p { margin: 4px 0 0; color: var(--webase-color-ink-muted); font-size: 12px; line-height: 1.4; }
  ...
  .ds-token-copy strong { color: var(--webase-color-ink); font-family: var(--webase-font-sans); font-size: 12px; letter-spacing: .06em; text-transform: uppercase; }
  code { color: var(--webase-color-brand); font-family: var(--webase-font-mono); font-size: 10px; line-height: 1.45; overflow-wrap: anywhere; }
  .ds-token-copy p, .ds-border-grid p, .ds-shadow-grid p, .ds-space-row p { margin: 0; color: var(--webase-color-ink-muted); font-size: 12px; line-height: 1.4; }
  .ds-ink-strip { display: flex; align-items: center; gap: 0; margin-top: 34px; border: 1px solid var(--webase-color-border); }
  .ds-ink-strip span { padding: 14px; color: var(--webase-color-ink-muted); font-family: var(--webase-font-mono); font-size: 10px; letter-spacing: .08em; text-transform: uppercase; }
  .ds-ink-strip span:nth-child(1) { background: var(--webase-color-surface); }
  .ds-ink-strip span:nth-child(2) { color: var(--webase-color-canvas); background: var(--webase-color-brand); }
  .ds-ink-strip span:nth-child(3) { color: var(--webase-color-canvas); background: var(--webase-color-ink); }
  .ds-ink-strip span:last-child { margin-left: auto; color: var(--webase-color-ink-muted); font-family: var(--webase-font-body); font-size: 13px; letter-spacing: 0; text-transform: none; }
  .ds-type-stack { border-top: 1px solid var(--webase-color-border-strong); }
  .ds-type-row { display: grid; grid-template-columns: 130px minmax(0, 1fr) 180px; gap: 18px; align-items: center; padding: 18px 0; border-bottom: 1px solid var(--webase-color-border); }
  .ds-type-label { color: var(--webase-color-ink-muted); font-family: var(--webase-font-sans); font-size: 11px; letter-spacing: .08em; text-transform: uppercase; }
  .ds-type-sample { overflow: hidden; color: var(--webase-color-ink); white-space: nowrap; text-overflow: ellipsis; }
  .ds-type-type { font-family: var(--webase-font-body); font-size: 26px; }
  .ds-type-row:nth-child(2) .ds-type-sample { font-family: var(--webase-font-display); font-size: 28px; font-style: italic; }
  .ds-type-row:nth-child(3) .ds-type-sample { font-family: var(--webase-font-sans); font-size: 14px; letter-spacing: .12em; text-transform: uppercase; }
  .ds-type-row:nth-child(4) .ds-type-sample { font-family: var(--webase-font-mono); font-size: 13px; }
  .ds-type-row p { grid-column: 2 / -1; margin: -8px 0 0; color: var(--webase-color-ink-muted); font-size: 12px; }
  .ds-scale-demo { display: grid; gap: 16px; margin-top: 34px; padding: 24px; border: 1px solid var(--webase-color-border); background: var(--webase-color-surface); }
  .ds-scale-hero { font-family: var(--webase-font-body); font-size: clamp(34px, 5vw, 58px); letter-spacing: -.06em; line-height: .9; }
  .ds-scale-title { font-family: var(--webase-font-body); font-size: 28px; }
  .ds-scale-body { max-width: 460px; color: var(--webase-color-ink-muted); font-size: 15px; line-height: 1.5; }
  .ds-scale-caption { color: var(--webase-color-brand); font-family: var(--webase-font-mono); font-size: 10px; letter-spacing: .12em; }
  .ds-prose-sheet { margin-top: 34px; border: 1px solid var(--webase-color-border-strong); background: var(--webase-color-surface); box-shadow: 7px 7px 0 color-mix(in srgb, var(--webase-color-brand) 8%, transparent); }
  .ds-prose-head { display: grid; grid-template-columns: minmax(0, 1fr) minmax(0, 1.6fr); gap: 16px 32px; align-items: end; padding: 20px 24px; border-bottom: 1px solid var(--webase-color-border-strong); }
  .ds-prose-kicker { margin: 0 0 6px; color: var(--webase-color-brand); font-family: var(--webase-font-mono); font-size: 9px; letter-spacing: .12em; text-transform: uppercase; }
  .ds-prose-head h3 { margin: 0; font-family: var(--webase-font-body); font-size: 28px; font-weight: 500; letter-spacing: -.04em; line-height: 1; }
  .ds-prose-head h3 code { font-size: 20px; }
  .ds-prose-head > p { margin: 0; color: var(--webase-color-ink-muted); font-size: 13px; line-height: 1.5; }
  ...
  .ds-space-row { display: grid; grid-template-columns: 75px 1fr 60px minmax(160px, 1fr); align-items: center; gap: 14px; min-height: 48px; border-bottom: 1px solid var(--webase-color-border); }
  .ds-space-row strong { color: var(--webase-color-ink); font-family: var(--webase-font-mono); font-size: 11px; font-weight: 400; }
  .ds-space-bar { display: block; width: var(--space-size); max-width: 100%; min-width: 4px; height: 10px; background: var(--webase-color-brand); }
  ...
  .ds-border-grid article { display: grid; grid-template-columns: 120px minmax(0, 1fr); align-items: center; gap: 10px 16px; padding: 18px; border: 1px solid var(--webase-color-border); }
  .ds-border-demo { grid-row: span 3; display: block; width: 100%; height: 62px; border: 1px solid var(--webase-color-border); }
  .ds-border-grid article:nth-child(2) .ds-border-demo { border-color: var(--webase-color-border-strong); }
  .ds-border-dots .ds-border-demo { border: 0; background-image: var(--webase-dot-rule-image); background-position: center; background-repeat: repeat-x; background-size: 7px 2px; }
  .ds-border-focus .ds-border-demo { border-color: var(--webase-color-brand); outline: 2px solid var(--webase-color-brand); outline-offset: 4px; }
  .ds-border-grid strong, .ds-motion-grid strong { color: var(--webase-color-ink); font-family: var(--webase-font-sans); font-size: 11px; letter-spacing: .06em; text-transform: uppercase; }
  .ds-shadow-grid article { min-height: 160px; padding: 20px; border: 1px solid var(--webase-color-border); background: var(--webase-color-surface); }
  .ds-shadow-grid article.ds-shadow-0 { box-shadow: 3px 3px 0 color-mix(in srgb, var(--webase-color-brand) 18%, transparent); }
  .ds-shadow-grid article.ds-shadow-1 { box-shadow: var(--webase-shadow-whisper); }
  .ds-shadow-grid article.ds-shadow-2 { box-shadow: 8px 8px 0 color-mix(in srgb, var(--webase-color-brand) 15%, transparent); }
  .ds-shadow-grid article span { display: block; margin-bottom: 22px; font-family: var(--webase-font-body); font-size: 28px; }
  .ds-component-ledger { display: flex; align-items: center; gap: 18px; margin: 8px 0 30px; padding: 12px 14px; border: 1px solid var(--webase-color-border); color: var(--webase-color-ink-muted); background: var(--webase-color-surface); font-family: var(--webase-font-mono); font-size: 9px; letter-spacing: .09em; text-transform: uppercase; }
  ...
  .ds-specimen-sheet > header > span { color: var(--webase-color-brand); font-family: var(--webase-font-mono); font-size: 10px; letter-spacing: .1em; }
  .ds-specimen-sheet > header h3 { margin: -5px 0 5px; font-family: var(--webase-font-body); font-size: 28px; font-weight: 500; letter-spacing: -.04em; line-height: 1; }
  .ds-specimen-sheet > header p { margin: 0; color: var(--webase-color-ink-muted); font-size: 12px; line-height: 1.45; }
  .ds-specimen-canvas { display: grid; gap: 26px; padding: 28px; border: 1px solid var(--webase-color-border); background: var(--webase-color-surface); box-shadow: 7px 7px 0 color-mix(in srgb, var(--webase-color-brand) 8%, transparent); }
  .ds-control-row { display: flex; flex-wrap: wrap; align-items: center; gap: 10px; }
  .ds-icon-button-row { padding-top: 20px; border-top: 1px dashed var(--webase-color-border-strong); }
  ...
  .ds-selection-grid > div { display: grid; align-content: start; gap: 14px; min-height: 170px; padding: 18px; border: 1px solid var(--webase-color-border); background: var(--webase-color-canvas); }
  .ds-group-label { margin: 0 0 4px; color: var(--webase-color-brand); font-family: var(--webase-font-mono); font-size: 9px; letter-spacing: .12em; text-transform: uppercase; }
  .ds-navigation-stack { gap: 28px; }
  .ds-navigation-stack > :global(nav:first-child) { padding-bottom: 18px; border-bottom: 1px dashed var(--webase-color-border-strong); }
  ...
  .ds-icon-grid { display: grid; grid-template-columns: repeat(4, 1fr); border-top: 1px solid var(--webase-color-border-strong); border-left: 1px solid var(--webase-color-border-strong); }
  .ds-icon-grid > div { display: grid; gap: 9px; min-height: 150px; padding: 20px; border-right: 1px solid var(--webase-color-border-strong); border-bottom: 1px solid var(--webase-color-border-strong); }
  .ds-icon-glyph { display: grid; width: 48px; height: 48px; place-items: center; border: 1px solid var(--webase-color-border); color: var(--webase-color-brand); background: var(--webase-color-surface); }
  .ds-icon-glyph :global(svg) { width: 24px; height: 24px; stroke-width: 1.6; }
  .ds-icon-grid strong { font-family: var(--webase-font-sans); font-size: 11px; letter-spacing: .08em; text-transform: uppercase; }
  .ds-motion-proof { border: 1px solid var(--webase-color-border-strong); background: var(--webase-color-surface); box-shadow: 8px 8px 0 color-mix(in srgb, var(--webase-color-brand) 9%, transparent); }
  .ds-motion-toolbar { display: flex; align-items: center; justify-content: space-between; gap: 18px; min-height: 70px; padding: 14px 18px; border-bottom: 1px solid var(--webase-color-border-strong); }
  .ds-motion-toolbar span, .ds-motion-grid small { color: var(--webase-color-brand); font-family: var(--webase-font-mono); font-size: 9px; letter-spacing: .11em; text-transform: uppercase; }
  .ds-motion-toolbar p { margin: 5px 0 0; color: var(--webase-color-ink-muted); font-size: 13px; }
  .ds-motion-grid { gap: 0; }
  .ds-motion-grid article { display: grid; grid-template-columns: minmax(118px, .8fr) 1.2fr; gap: 20px; min-height: 205px; padding: 18px; border: 0; border-right: 1px solid var(--webase-color-border); border-bottom: 1px solid var(--webase-color-border); background: transparent; }
  ...
  .ds-motion-grid code { color: var(--webase-color-brand); font-size: 10px; }
  .ds-motion-grid p { margin: auto 0 3px; color: var(--webase-color-ink-muted); font-size: 12px; line-height: 1.45; }
  .ds-motion-visual { position: relative; display: grid; min-height: 150px; overflow: hidden; place-items: center; border: 1px dashed var(--webase-color-border-strong); background: var(--webase-color-canvas); }
  .ds-motion-press span { display: grid; width: 62px; height: 62px; place-items: center; border: 1px solid var(--webase-color-brand); border-radius: 50%; color: var(--webase-color-brand); font-family: var(--webase-font-mono); font-size: 9px; text-transform: uppercase; animation: ds-motion-press 1.5s var(--webase-ease-out) 120ms both; }
  .ds-motion-reveal i { position: absolute; inset: 20px 16px; border: 1px solid var(--webase-color-border); background: repeating-linear-gradient(transparent 0 17px, var(--webase-color-border) 18px); }
  .ds-motion-reveal span { z-index: 1; width: 86px; padding: 27px 12px; color: var(--webase-color-canvas); background: var(--webase-color-brand); font-family: var(--webase-font-body); font-style: italic; text-align: center; transform-origin: left; animation: ds-motion-reveal 900ms var(--webase-ease-out) 240ms both; }
  .ds-motion-register i, .ds-motion-register b { position: absolute; width: 68px; height: 68px; border: 1px solid var(--webase-color-brand); border-radius: 50%; }
  .ds-motion-register i { animation: ds-register-left 900ms var(--webase-ease-out) 380ms both; }
  .ds-motion-register b { border-radius: 0; animation: ds-register-right 900ms var(--webase-ease-out) 380ms both; }
  .ds-motion-register span { width: 7px; height: 7px; border-radius: 50%; background: var(--webase-color-brand); }
  .ds-motion-stagger { align-content: center; gap: 10px; padding: 0 18px; }
  .ds-motion-stagger i { width: 100%; height: 4px; background: var(--webase-color-brand); transform-origin: left; animation: ds-stagger 420ms var(--webase-ease-out) both; }
  ...
  .ds-motion-tape { display: grid; grid-template-columns: auto 1fr auto 1fr auto 3fr auto; align-items: center; gap: 9px; padding: 13px 18px; color: var(--webase-color-ink-muted); font-family: var(--webase-font-mono); font-size: 9px; }
  .ds-motion-tape i { height: 1px; background: var(--webase-color-brand); opacity: .45; }
  .ds-motion-rules { display: grid; grid-template-columns: repeat(3, 1fr); gap: 22px; margin-top: 30px; }
  .ds-motion-rules p { margin: 0; padding-top: 11px; border-top: 1px solid var(--webase-color-border-strong); color: var(--webase-color-ink-muted); font-size: 12px; line-height: 1.5; }
  .ds-motion-rules strong { display: block; margin-bottom: 6px; color: var(--webase-color-ink); font-family: var(--webase-font-sans); font-size: 10px; letter-spacing: .08em; text-transform: uppercase; }
  @keyframes ds-motion-press { 0%, 24% { transform: translateY(-5px); box-shadow: 0 7px 0 color-mix(in srgb, var(--webase-color-brand) 18%, transparent); } 40%, 58% { transform: translateY(2px) scale(.96); box-shadow: 0 0 0 transparent; } 76%, 100% { transform: none; box-shadow: 0 4px 0 color-mix(in srgb, var(--webase-color-brand) 12%, transparent); } }
  ...
  .ds-material-grid article { position: relative; min-height: 190px; padding: 22px; overflow: hidden; border: 1px solid var(--webase-color-border); }
  .ds-material-grid article > span { position: relative; z-index: 1; color: var(--webase-color-ink); font-family: var(--webase-font-body); font-size: 28px; }
  .ds-material-grid article > p { position: relative; z-index: 1; max-width: 26ch; margin-top: 18px; color: var(--webase-color-ink-muted); font-size: 13px; line-height: 1.45; }
  .ds-material-paper { background-color: var(--webase-color-surface); background-image: radial-gradient(circle at 15% 20%, color-mix(in srgb, var(--webase-color-brand) 8%, transparent) 0 1px, transparent 1px), radial-gradient(circle at 72% 58%, color-mix(in srgb, var(--webase-color-ink) 8%, transparent) 0 1px, transparent 1px); background-size: 15px 15px, 21px 21px; }
  .ds-material-ink { background: color-mix(in srgb, var(--webase-color-brand) 88%, var(--webase-color-ink)); }
  .ds-material-grid .ds-material-ink > span, .ds-material-grid .ds-material-ink > p { color: var(--webase-color-canvas); }
  ...
  .ds-material-register { background: var(--webase-color-canvas); }
  .ds-material-register i, .ds-material-register b { position: absolute; width: 90px; height: 90px; border: 1px solid var(--webase-color-brand); border-radius: 50%; content: ''; opacity: .6; }
  ...
  .ds-material-emboss { background: color-mix(in srgb, var(--webase-color-canvas) 72%, var(--webase-color-surface)); }
  .ds-material-emboss > i { position: absolute; right: 28px; bottom: 18px; width: 92px; height: 92px; border: 1px solid var(--webase-emboss-dark); border-radius: 50%; box-shadow: -1px -1px 1px var(--webase-emboss-light), 1px 1px 1px var(--webase-emboss-dark), inset -1px -1px 1px var(--webase-emboss-light), inset 1px 1px 1px var(--webase-emboss-dark); }
  .ds-material-emboss > i::after { position: absolute; inset: 15px; border: 1px solid var(--webase-emboss-dark); border-radius: 50%; box-shadow: -1px -1px 0 var(--webase-emboss-light), 1px 1px 0 var(--webase-emboss-dark); content: ''; }
  .ds-material-carbon { background: color-mix(in srgb, var(--webase-color-brand) 9%, var(--webase-color-canvas)); }
  .ds-material-grid .ds-material-carbon > p { color: var(--webase-color-ink); opacity: .72; }
  .ds-material-carbon > i { position: absolute; right: 16px; bottom: 17px; color: color-mix(in srgb, var(--webase-color-brand) 42%, transparent); font-family: var(--webase-font-mono); font-size: 27px; font-style: normal; font-weight: 700; letter-spacing: .08em; line-height: .9; text-shadow: 3px 2px 0 color-mix(in srgb, var(--webase-color-brand) 20%, transparent); transform: rotate(-3deg); }
  .ds-material-vellum { background: repeating-linear-gradient(90deg, var(--webase-color-canvas) 0 33px, var(--webase-color-border) 34px); }
  .ds-material-vellum::after { position: absolute; inset: 36px 20px 16px 42%; border: 1px solid var(--webase-emboss-light); background: color-mix(in srgb, var(--webase-color-surface) 62%, transparent); box-shadow: 3px 3px 12px var(--webase-shadow-book); backdrop-filter: blur(2px); content: ''; transform: rotate(3deg); }
  .ds-material-vellum > i { position: absolute; right: 36px; bottom: 38px; z-index: 1; color: var(--webase-color-brand); font-family: var(--webase-font-mono); font-size: 18px; font-style: normal; letter-spacing: .1em; line-height: 1.1; transform: rotate(3deg); }
  .ds-material-overprint { grid-column: 1 / -1; background-color: var(--webase-color-surface); background-image: radial-gradient(circle, color-mix(in srgb, var(--webase-color-ink) 28%, transparent) 0 1px, transparent 1.5px); background-size: 7px 7px; }
  .ds-material-overprint i, .ds-material-overprint b { position: absolute; right: 38px; bottom: 20px; width: 94px; height: 94px; border-radius: 50%; background: var(--webase-color-brand); opacity: .66; mix-blend-mode: multiply; }
  .ds-material-overprint b { right: 88px; background: color-mix(in srgb, var(--webase-color-ink) 76%, var(--webase-color-canvas)); opacity: .46; }
  .ds-endnote { display: flex; align-items: end; justify-content: space-between; gap: 24px; padding-top: 24px; }
  .ds-endnote p { max-width: 420px; margin: 0; color: var(--webase-color-ink-muted); font-size: 14px; line-height: 1.5; }
  ...
  .ds-api-canvas { gap: 0; }  .ds-api-note { max-width: 62ch; margin: 0 0 8px; color: var(--webase-color-ink-muted); font-size: 13px; line-height: 1.5; }
  .ds-api-entry { padding: 20px 0; border-top: 1px solid var(--webase-color-border); }
  .ds-api-entry h4 { margin: 0 0 4px; color: var(--webase-color-brand); font-family: var(--webase-font-mono); font-size: 11px; font-weight: 400; letter-spacing: .08em; }
  .ds-api-entry > p { max-width: 62ch; margin: 0 0 14px; color: var(--webase-color-ink-muted); font-size: 13px; line-height: 1.45; }
  .ds-api-entry dl { display: grid; gap: 0; margin: 0; }
  .ds-api-entry dl > div { display: grid; grid-template-columns: minmax(0, 200px) minmax(0, 1fr); gap: 8px 20px; padding: 7px 0; border-top: 1px dashed var(--webase-color-border); }
  ...
  .ds-api-entry dd code { color: var(--webase-color-ink-muted); }
  .ds-api-entry dd em { color: var(--webase-color-ink); font-family: var(--webase-font-mono); font-size: 10px; font-style: normal; }
  .ds-api-entry abbr { color: var(--webase-color-error); font-size: 12px; text-decoration: none; }
  .ds-api-bind { padding: 1px 5px; border: 1px solid var(--webase-color-border-strong); color: var(--webase-color-ink-muted); font-family: var(--webase-font-mono); font-size: 8px; letter-spacing: .09em; text-transform: uppercase; }
