<script lang="ts">
  ...
  import XueAccordion from '$lib/components/design-system/XueAccordion.svelte';
  import XueAlert from '$lib/components/design-system/XueAlert.svelte';
  import XueBadge from '$lib/components/design-system/XueBadge.svelte';
  import XueBreadcrumbs from '$lib/components/design-system/XueBreadcrumbs.svelte';
  import XueButton from '$lib/components/design-system/XueButton.svelte';
  import XueCard from '$lib/components/design-system/XueCard.svelte';
  import XueCheck from '$lib/components/design-system/XueCheck.svelte';
  import XueDialog from '$lib/components/design-system/XueDialog.svelte';
  import XueDivider from '$lib/components/design-system/XueDivider.svelte';
  import XueEmptyState from '$lib/components/design-system/XueEmptyState.svelte';
  import XueField from '$lib/components/design-system/XueField.svelte';
  import XueIconButton from '$lib/components/design-system/XueIconButton.svelte';
  import XueIcon from '$lib/components/design-system/XueIcon.svelte';
  import XueLoader from '$lib/components/design-system/XueLoader.svelte';
  import XueProgress from '$lib/components/design-system/XueProgress.svelte';
  import XuePagination from '$lib/components/design-system/XuePagination.svelte';
  import XueRadio from '$lib/components/design-system/XueRadio.svelte';
  import XueSelect from '$lib/components/design-system/XueSelect.svelte';
  import XueSkeleton from '$lib/components/design-system/XueSkeleton.svelte';
  import XueSlider from '$lib/components/design-system/XueSlider.svelte';
  import XueSwitch from '$lib/components/design-system/XueSwitch.svelte';
  import XueTabs from '$lib/components/design-system/XueTabs.svelte';
  import XueTextarea from '$lib/components/design-system/XueTextarea.svelte';
  import XueToast from '$lib/components/design-system/XueToast.svelte';
  import XueTooltip from '$lib/components/design-system/XueTooltip.svelte';
  ...
  function showToast() {
    toastOpen = true;
    window.setTimeout(() => (toastOpen = false), 4200);
  }
</script>

<Seo
  title="Design System"
  description="A living catalogue of the Will Xue visual language, components, and interaction notes."
  path="/design-system"
  noindex={true}
 />
...
<main class="ds-page">
  ...
  <div class="ds-content">
    <section id="overview" class="ds-hero ds-section">
      <div class="ds-hero-grid">
        <div>
          ...
          <div class="ds-hero-ledger" aria-label="Design system coverage"><span><strong>24</strong> components</span><span><strong>10</strong> foundations</span><span><strong>3</strong> input modes</span></div>
        </div>
      ...
      <XueDivider label="Read the specimen" />
    ...
    <section id="color" class="ds-section">
      <header class="ds-section-head"><p class="ds-kicker">01 / Foundation</p><h2>Color</h2><p>{sections[1].note}</p></header>
      <div class="ds-token-grid ds-color-grid">
        {#each colors as token (token.name)}
          <article class="ds-token ds-color-token">
            ...
            <div class="ds-token-copy"><div><strong>{token.name}</strong><XueBadge label="Canonical" /></div><code>{token.value}</code><p>{token.note}</p></div>
          </article>
        {/each}
      </div>
    ...
    <section id="type" class="ds-section">
      <header class="ds-section-head"><p class="ds-kicker">02 / Voice</p><h2>Typography</h2><p>{sections[2].note}</p></header>
    ...
    <section id="space" class="ds-section">
      <header class="ds-section-head"><p class="ds-kicker">03 / Rhythm</p><h2>Spacing</h2><p>{sections[3].note}</p></header>
    ...
    <section id="borders" class="ds-section">
      <header class="ds-section-head"><p class="ds-kicker">04 / Edges</p><h2>Borders</h2><p>{sections[4].note}</p></header>
    ...
    <section id="shadows" class="ds-section">
      <header class="ds-section-head"><p class="ds-kicker">05 / Depth</p><h2>Shadows</h2><p>{sections[5].note}</p></header>
    ...
    <section id="components" class="ds-section">
      <header class="ds-section-head"><p class="ds-kicker">06 / Instruments</p><h2>Components</h2><p>{sections[6].note}</p></header>
      <div class="ds-component-ledger"><span>07 specimen sheets</span><span>Default + focus + disabled + error</span><XueBadge label="Expanded" dot /></div>

      <article class="ds-specimen-sheet">
        <header><span>01</span><div><h3>Actions</h3><p>Clear hierarchy, compact language, and visible working states.</p></div><XueBadge label="Canonical" /></header>
        <div class="ds-specimen-canvas">
          <div class="ds-control-row"><XueButton label="Save note" variant="ink" icon="check" onclick={showToast} /><XueButton label="Open archive" variant="outline" icon="arrow" /><XueButton label="Add entry" variant="quiet" icon="plus" /><XueButton label="Read note" variant="text" /><XueButton label="Saving" loading /><XueButton label="Unavailable" disabled /></div>
          <div class="ds-control-row ds-icon-button-row"><XueIconButton label="Open specimen" /><XueIconButton label="Bookmark specimen" icon="bookmark" bind:pressed={saved} /><XueIconButton label="Like specimen" icon="heart" /><XueIconButton label="Add specimen" icon="plus" /><XueIconButton label="Disabled action" disabled /></div>
        </div>
      </article>

      <article class="ds-specimen-sheet">
        <header><span>02</span><div><h3>Content surfaces</h3><p>Editorial cards, loading placeholders, and a useful empty drawer.</p></div><XueBadge label="Canonical" /></header>
        <div class="ds-specimen-canvas">
          <div class="ds-card-grid"><XueCard title="A paper surface" body="A card is a small page with enough structure to hold one thought." meta="Updated 02 Aug" action="Read note" /><XueCard title="Ink in reverse" body="Use the seal field sparingly, when an object needs to step forward." variant="ink" meta="Featured" action="Open" /></div>
          <div class="ds-content-state-grid"><XueSkeleton rows={3} /><XueEmptyState /></div>
        </div>
      </article>

      <article class="ds-specimen-sheet">
        <header><span>03</span><div><h3>Fields</h3><p>Help, validation, counters, typeahead, and direct manipulation.</p></div><XueBadge label="Interactive" tone="experiment" /></header>
        <div class="ds-specimen-canvas">
          <div class="ds-form-grid">
            <XueField id="ds-demo-field" label="Field note" bind:value={fieldValue} help="Plain language and visible focus." required />
            <XueField id="ds-error-field" label="Edition code" value="proof-" error="Use a complete code, for example proof-02." />
            <XueSelect id="ds-demo-select" label="Material" bind:value={selectValue} help="Arrow keys and typeahead are supported." options={[{ label: 'Paper', value: 'paper' }, { label: 'Ink wash', value: 'ink' }, { label: 'Blue mark', value: 'blue' }]} />
            <XueSlider id="ds-demo-slider" label="Ink density" bind:value={sliderValue} />
            <XueTextarea id="ds-demo-textarea" label="Longer note" bind:value={messageValue} help="Keep the note specific enough to be useful later." maxLength={180} />
          </div>
        </div>
      </article>

      <article class="ds-specimen-sheet">
        <header><span>04</span><div><h3>Selection</h3><p>Binary, single-choice, mixed, and annotated controls.</p></div><XueBadge label="Canonical" /></header>
        <div class="ds-specimen-canvas ds-selection-grid">
          <div><p class="ds-group-label">Checkbox</p><XueCheck label="Remember this specimen" description="Keep the proof in the local drawer." bind:checked /><XueCheck label="Partially archived" indeterminate /><XueCheck label="Unavailable choice" disabled /></div>
          <div><p class="ds-group-label">Radio</p><XueRadio name="ds-radio" label="First edition" value="first" bind:selected={radioValue} /><XueRadio name="ds-radio" label="Second edition" value="second" bind:selected={radioValue} /><XueRadio name="ds-radio" label="Retired edition" value="retired" bind:selected={radioValue} disabled /></div>
          <div><p class="ds-group-label">Switch</p><XueSwitch label="Show annotations" description="Reveal proofing notes in the margin." bind:checked={switchValue} /><XueSwitch label="Locked setting" disabled /></div>
        </div>
      </article>

      <article class="ds-specimen-sheet">
        <header><span>05</span><div><h3>Navigation</h3><p>Location, local view state, and movement through long archives.</p></div><XueBadge label="Keyboard ready" tone="experiment" /></header>
        <div class="ds-specimen-canvas ds-navigation-stack">
          <XueBreadcrumbs items={[{ label: 'Archive', href: '/' }, { label: 'Field notes', href: '/blog' }, { label: 'Paper systems' }]} />
          <XueTabs id="ds-demo-tabs" label="Specimen views" items={['Overview', 'Notes', 'Archive']} panels={['A summary of the current specimen.', 'Annotations and decisions attached to the proof.', 'Earlier editions and retired states.']} />
          <XuePagination total={5} bind:page={pageValue} label="Archive pages" />
        </div>
      </article>

      <article class="ds-specimen-sheet">
        <header><span>06</span><div><h3>Feedback</h3><p>Status should explain what happened and what remains possible.</p></div><XueBadge label="Live region" tone="experiment" /></header>
        <div class="ds-specimen-canvas">
          <div class="ds-alert-grid"><XueAlert title="Proof saved" message="The local specimen now includes your latest annotations." tone="success" dismissible bind:open={alertOpen} /><XueAlert title="Image still processing" message="You can keep editing while the large preview is generated." tone="warning" /></div>
          <div class="ds-feedback-grid"><div><XueProgress value={68} label="Archive completeness" /><XueProgress value={92} label="Metadata quality" tone="success" compact /></div><div class="ds-loader-row"><XueLoader label="Riffle" /><XueTooltip id="ds-demo-tooltip" label="Hover or focus" text="Tooltips clarify a mark without interrupting the page." /></div></div>
          <div class="ds-control-row ds-action-row"><XueButton label="Show toast" variant="outline" onclick={showToast} /><XueButton label="Open dialog" variant="ink" onclick={(event) => { (event.currentTarget as HTMLButtonElement).focus(); dialogOpen = true; }} /></div>
        </div>
      </article>

      <article class="ds-specimen-sheet">
        <header><span>07</span><div><h3>Disclosure</h3><p>Long explanations stay available without making every page dense.</p></div><XueBadge label="Keyboard ready" tone="experiment" /></header>
        <div class="ds-specimen-canvas"><XueAccordion id="ds-demo-accordion" bind:open={accordionOpen} items={[{ title: 'When should blue ink appear?', content: 'Use blue for links, active controls, focus, and explicit registration marks. It should guide the eye, not tint every surface.' }, { title: 'How should motion behave?', content: 'Motion confirms a change or preserves spatial context. Reduced-motion users receive the same state change without travel or rotation.' }, { title: 'Can these components replace the site today?', content: 'They are ready for review here. Existing pages remain unchanged until a separate migration pass is approved.' }]} /></div>
      </article>
    </section>

    <section id="icons" class="ds-section">
      <header class="ds-section-head"><p class="ds-kicker">07 / Marks</p><h2>Iconography</h2><p>{sections[7].note}</p></header>
      <div class="ds-icon-grid">
        <div><span class="ds-icon-glyph"><XueIcon name="arrow-up-right" size={24} strokeWidth={1.6} /></span><strong>External</strong><code>ArrowUpRight</code></div>
        <div><span class="ds-icon-glyph"><XueIcon name="search" size={24} strokeWidth={1.6} /></span><strong>Search</strong><code>Search</code></div>
        <div><span class="ds-icon-glyph"><XueIcon name="sparkles" size={24} strokeWidth={1.6} /></span><strong>Mark</strong><code>Sparkles</code></div>
        <div><span class="ds-icon-glyph"><XueIcon name="x" size={24} strokeWidth={1.6} /></span><strong>Close</strong><code>X</code></div>
        <div><span class="ds-icon-glyph"><XueIcon name="archive" size={24} strokeWidth={1.6} /></span><strong>Archive</strong><code>Archive</code></div>
        <div><span class="ds-icon-glyph"><XueIcon name="bookmark" size={24} strokeWidth={1.6} /></span><strong>Save</strong><code>Bookmark</code></div>
        <div><span class="ds-icon-glyph"><XueIcon name="settings-2" size={24} strokeWidth={1.6} /></span><strong>Adjust</strong><code>Settings2</code></div>
        <div><span class="ds-icon-glyph"><XueIcon name="zoom-in" size={24} strokeWidth={1.6} /></span><strong>Inspect</strong><code>ZoomIn</code></div>
      </div>
    ...
    <section id="motion" class="ds-section">
      <header class="ds-section-head"><p class="ds-kicker">08 / Time</p><h2>Motion</h2><p>{sections[8].note}</p></header>
      <div class="ds-motion-proof">
        <div class="ds-motion-toolbar">
          ...
          <button type="button" onclick={() => (motionReplay += 1)}><XueIcon name="sparkles" size={15} />Replay motion sequence</button>
        </div>
        {#key motionReplay}
    ...
    <section id="materials" class="ds-section">
      <header class="ds-section-head"><p class="ds-kicker">09 / Surface</p><h2>Materials</h2><p>{sections[9].note}</p></header>
  ...
  <div class="ds-floating-feedback"><XueToast bind:open={toastOpen} /><XueDialog bind:open={dialogOpen} /></div>
</main>
...
<style>
  ...
  .ds-section-head { display: grid; grid-template-columns: 1fr minmax(0, 2fr); align-items: end; column-gap: 32px; margin-bottom: 34px; }
  .ds-section-head .ds-kicker { grid-column: 1 / -1; margin-bottom: 12px; }
  .ds-section-head h2 { margin-bottom: 0; font-family: var(--font); font-size: clamp(38px, 6vw, 72px); font-weight: 500; letter-spacing: -.06em; line-height: .9; }
  .ds-section-head > p:last-child { max-width: 280px; margin: 0; color: var(--ink-muted); font-size: 14px; line-height: 1.45; }
  ...
  .ds-type-undefined { font-family: var(--font); }
  ...
  .ds-component-block { display: grid; gap: 22px; padding: 22px 0; border-bottom: 1px solid var(--hairline); }
  .ds-component-meta { display: flex; align-items: center; justify-content: space-between; gap: 16px; color: var(--ink-muted); font-family: var(--mono); font-size: 10px; letter-spacing: .12em; text-transform: uppercase; }
  ...
  .ds-component-grid { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 32px; }
  ...
  .ds-state-row { padding: 4px 0 28px; }
  ...
  .ds-material-emboss > i { position: absolute; right: 28px; bottom: 18px; width: 92px; height: 92px; border: 1px solid rgb(20 20 19 / 12%); border-radius: 50%; box-shadow: -1px -1px 1px rgb(255 255 255 / 72%), 1px 1px 1px rgb(20 20 19 / 20%), inset -1px -1px 1px rgb(255 255 255 / 72%), inset 1px 1px 1px rgb(20 20 19 / 16%); }
  .ds-material-emboss > i::after { position: absolute; inset: 15px; border: 1px solid rgb(20 20 19 / 10%); border-radius: 50%; box-shadow: -1px -1px 0 rgb(255 255 255 / 62%), 1px 1px 0 rgb(20 20 19 / 16%); content: ''; }
  ...
  .ds-material-vellum::after { position: absolute; inset: 36px 20px 16px 42%; border: 1px solid rgb(255 255 255 / 55%); background: color-mix(in srgb, var(--surface) 62%, transparent); box-shadow: 3px 3px 12px rgb(20 20 19 / 9%); backdrop-filter: blur(2px); content: ''; transform: rotate(3deg); }
  ...
  .ds-floating-feedback :global(.ds-dialog-scrim) { pointer-events: auto; }
  @media (prefers-reduced-motion: reduce) { .ds-cursor, .ds-motion-visual * { animation: none !important; } }
  @media (max-width: 1100px) { .ds-selection-grid { grid-template-columns: 1fr 1fr; } .ds-selection-grid > div:last-child { grid-column: 1 / -1; } }
