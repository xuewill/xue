<script lang="ts">
  import { WeBaseBadge, WeBaseButton, WeBaseIcon, WeBaseTag } from '@webaseui/svelte';
...
<main class="page listing-page archive-page">
  <header class="archive-header">
    ...
    <WeBaseBadge
      class="archive-count"
      label={`${visibleEntries.length.toString().padStart(2, '0')} entries`}
      dot
    />
  </header>
  ...
  <nav class="archive-filters" aria-label="Filter archive">
    {#each kinds as kind (kind.value)}
      <WeBaseButton
        size="sm"
        variant={activeKind === kind.value ? 'ink' : 'text'}
        label={kind.label}
          ...
          {#each entries as entry (entry.kind + entry.slug)}
            <a class={`archive-entry archive-${entry.kind}`} href={entry.href}>
              ...
              <span class="archive-entry-main">
                <WeBaseTag
                  variant="outline"
                  label={entry.kind === 'post' ? 'Writing' : entry.kind === 'project' ? 'Project' : 'Album'}
                />
              ...
              {/if}
              <span class="archive-arrow" aria-hidden="true"><WeBaseIcon name="arrow-up-right" size={18} /></span>
            </a>
          {/each}
        </div>
      </section>
...
<style>
  ...
  .archive-header h1 { max-width: 620px; margin: 12px 0 16px; font-family: var(--webase-font-body); font-size: clamp(38px, 7vw, 78px); font-weight: 500; letter-spacing: -0.045em; line-height: .95; }
  .archive-intro { max-width: 520px; margin: 0; color: var(--webase-color-ink-muted); font-size: 16px; line-height: 1.55; }
  .archive-count { white-space: nowrap; }
  .archive-filters { display: flex; gap: 8px; margin-bottom: 34px; padding-bottom: 12px; background-image: var(--webase-dot-rule-image); background-position: left bottom; background-repeat: repeat-x; background-size: 7px 2px; }
  .archive-timeline { background-image: var(--webase-dot-rule-image); background-position: left top; background-repeat: repeat-x; background-size: 7px 2px; }
  .archive-year { display: grid; grid-template-columns: 100px 1fr; gap: 30px; padding: 28px 0 42px; background-image: var(--webase-dot-rule-image); background-position: left bottom; background-repeat: repeat-x; background-size: 7px 2px; }
  .archive-year h2 { margin: 0; color: var(--webase-color-brand); font-family: var(--webase-font-mono); font-size: 15px; font-weight: 400; letter-spacing: .08em; }
  .archive-entries { display: grid; gap: 0; }
  .archive-entry { display: grid; grid-template-columns: 88px minmax(0, 1fr) 96px 20px; align-items: center; gap: 18px; min-height: 126px; padding: 18px 0; color: var(--webase-color-ink); background-image: var(--webase-dot-rule-image); background-position: left top; background-repeat: repeat-x; background-size: 7px 2px; }
  .archive-entry:first-child { background-image: none; }
  .archive-entry-date { align-self: start; padding-top: 4px; color: var(--webase-color-ink-muted); font-family: var(--webase-font-mono); font-size: 11px; }
  .archive-entry-main { display: grid; gap: 5px; min-width: 0; }
  .archive-entry-main :global(.ds-tag) { width: max-content; }
  .archive-entry strong { overflow: hidden; font-family: var(--webase-font-body); font-size: clamp(20px, 2.7vw, 30px); font-weight: 500; text-overflow: ellipsis; white-space: nowrap; }
  .archive-entry-description { overflow: hidden; color: var(--webase-color-ink-muted); font-size: 14px; line-height: 1.4; text-overflow: ellipsis; white-space: nowrap; }
  .archive-entry-meta { color: var(--webase-color-ink-muted); font-family: var(--webase-font-sans); font-size: 11px; letter-spacing: .03em; }
  .archive-entry img { width: 96px; height: 76px; object-fit: cover; filter: saturate(.86); }
  .archive-arrow { color: var(--webase-color-brand); font-size: 18px; transition: transform var(--webase-duration-ui) var(--webase-ease-out); }
  .archive-entry:hover strong, .archive-entry:focus-visible strong { color: var(--webase-color-brand); }
